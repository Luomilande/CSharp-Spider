...
namespace HDemo
{
    ...
    public class Html2Article
    {
        ...
        {
            new[] { @"(?im)<style+?[\s\S]*?>[\s\S]*?</style>", "" },
            new[] { @"(?im)<script+?[\s\S]*?>[\s\S]*?</script>", "" },
            new[] { @"(?is)<!--.*?-->", "" },    // 过滤Html代码中的注释
        ...
        private static string GetTitle(string html)
        {
            ...
            string h1Filter = @"<h1+?[\s\S]*?>[\s\S]*?</h1>";
            string h2Filter = @"<h2+?[\s\S]*?>[\s\S]*?</h2>";
            string h3Filter= @"<h3+?[\s\S]*?>[\s\S]*?</h3>";
            string spanFilter= @"<span+?[\s\S]*?>[\s\S]*?</span>";
            string clearFilter = @"<.*?>";
            string title = "";
            string content = "";
            ...
            if (match.Success)
            {
                Match spanmatch = Regex.Match(match.Groups[0].Value, spanFilter, RegexOptions.IgnoreCase);
                if (spanmatch.Success) content = Regex.Replace(spanmatch.Groups[0].Value, clearFilter, "").Trim();
                else content = Regex.Replace(match.Groups[0].Value, clearFilter, "").Trim();
                if (!String.IsNullOrEmpty(content) && title.StartsWith(content))
                {
            ...
            // h1匹配不了试着匹配h2
            match = Regex.Match(html, h2Filter, RegexOptions.IgnoreCase);
            if (match.Success)
            {
                Match spanmatch = Regex.Match(match.Groups[0].Value, spanFilter, RegexOptions.IgnoreCase);
                if (spanmatch.Success) content = Regex.Replace(spanmatch.Groups[0].Value, clearFilter, "").Trim();
            ...
            // h1,h2匹配不了试着匹配h3
            match = Regex.Match(html, h3Filter, RegexOptions.IgnoreCase);
            if (match.Success)
            {
                Match spanmatch = Regex.Match(match.Groups[0].Value, spanFilter, RegexOptions.IgnoreCase);
                if (spanmatch.Success) content = Regex.Replace(spanmatch.Groups[0].Value, clearFilter, "").Trim();
        ...
        private static DateTime GetPublishDate(string html)
        {
            ...
            Match match = Regex.Match(
                text,
                @"((\d{4}|\d{2})(\-|\/|\\)\d{1,2}(\-|\/|\\)\d{1,2})(\s?\d{2}:\d{2})?|(\d{4}年\d{1,2}月\d{1,2}日)(\s?\d{2}:\d{2})?",
